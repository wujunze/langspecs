...
**变量**是包含PHP值的命名数据存储区域。  变量由[VSlot](04-basic-concept.md#general)表示。  变量是通过[赋值](04-basic-concept.md#assign)创建的。
变量通过*unset*被销毁，或者通过显式调用[`unset`语句](11-statement.md#the-unset语句)，或者通过引擎销毁。内部的[`isset`](10-expression.md#isset)测试给定变量是否存在，是否设置为 'NULL'。
如果表达式中使用了一个变量(到目前为止还没有定义)，则应用不同的策略来确定变量是隐式定义的，还是使用替代值，以及是否发出通知。策略取决于变量的类型以及使用未定义变量的上下文。这些策略将在下面的不同[变量种类](#变量种类)小节中详细说明。
